...
for i in range(5):
    ...
    for x in itertools.combinations(categories_ids, i+1):
        combinations[i+1].append(x)


def level_generator(n, c, display = False):
    '''
    ...
    Arg:
        ...
        display: bool, parameter to display or not the selected images.
    '''
...
with open('level.pkl', 'wb') as input:
    pickle.dump(level_generator(int(sys.argv[1]), int(sys.argv[2])), input)
